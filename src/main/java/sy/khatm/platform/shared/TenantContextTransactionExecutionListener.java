 ...
 *
 * <p>Resolves {@link TenantContext#current()} — which falls back to {@link
 * TenantContext#DEFAULT_TENANT_ID} when nothing set it explicitly — so every pre-KH-2.1 code path
 * (seeders, the default tenant's own traffic, any test that never touches tenant machinery) keeps
 * working unchanged under RLS: {@code app.tenant_id} is always set to *something* correct for the
 * calling thread, never left unset.
 */
